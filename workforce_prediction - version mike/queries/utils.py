...
def prepare_data(data, year=2019, date=None): 
    """
    ...
    if date==None:
        ...
        data_train = data.loc[data.index<date,:]
        data_test = data.loc[data.index>=date,:]
...
def make_verif(forecast, data_train, data_test): 
    ...
    forecast_tmp.index = pd.to_datetime(forecast_tmp.ds)
    
    data_train_tmp.index = pd.to_datetime(data_train_tmp.ds)
    data_test_tmp.index = pd.to_datetime(data_test_tmp.ds)
    data = pd.concat([data_train_tmp, data_test_tmp], axis=0)
    
    forecast_tmp.loc[:,'y'] = data.loc[:,'y']
    
    return forecast_tmp

def plot_verif(verif, year=2019, date=None):
    """
    ...
    if date == None:
        ...
        train = verif.loc[verif.index<date,:]
    ...
    if date == None:
        ...
        test = verif.loc[verif.index>=date, :]
    ...
    if date == None:
        ...
        ax.axvline(date, color='0.8', alpha=0.7)
    
    ax.grid(ls=':', lw=0.5)
    
    return f

def plot_verif_component(verif, component='rain', year=2019): 
    """
...
def plot_joint_plot(verif, x='yhat', y='y', title=None, fpath = 'figures', fname = None): 
    """
